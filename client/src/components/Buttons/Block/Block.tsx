...
const Block: React.FunctionComponent<Props> = ({
    ...
    return (
        <button role={role} extra-css={attribute} onClick={() => {
            if (isLoading) return;
            if (!callback) return;
            callback();
        }} className={`${styles.btn} ${styles[family]}`}>
            {isLoading && <svg viewBox="0 0 28 28" fill="none" xmlns="http://www.w3.org/2000/svg">
